...
class ParsedUrl {
   ...
   */
  constructor (urlToParse) {
    let ljr = LeadingJunkRE.exec(urlToParse)
    let tjr = TrailingJunkRe.exec(ljr.groups.rest)
    let urlr = URLRe.exec(tjr.groups.rest)
    this.leadingJunk = ljr.groups.junk
    this.trailingJunk = tjr.groups.junk
    this.scheme = urlr.groups.scheme || ''
    this.colonAfterScheme = urlr.groups.colonAfterScheme || ''
    this.questionMark = urlr.groups.questionMark || ''
    this.query = urlr.groups.query || ''
    this.hashSign = urlr.groups.hashSign || ''
    this.fragment = urlr.groups.fragment || ''
    this.slashes = ''
    this.username = ''
    ...
    if (urlr.groups.pathish) {
      Parser.parsePathish(this, urlr.groups.pathish)
    }
  }
...
class Parser {
   ...
   * @return {ParsedUrl}
   */
  static parsePathish (url, pathish) {
    ...
    if (cleanScheme === 'file') {
      ...
      if (m != null) {
        ...
        const { v4, v6 } = Parser.parseIpV4Or6(url.host)
        url.ip4 = v4
        url.ip6 = v6
        // url.ip4, (url.ip6 = Parser.parseIpV4Or6(url.host))
  ...
  static parseIpV4Or6 (ipString) {
    ...
    try {
      if (ipString.startsWith('[') && ipString.endsWith(']')) {
        ...
        results.v4 = Parser.parseIPV4(ipString)
      }
    } catch (e) {}
    return results
  }
  ...
  static parseIPV4 (ipString) {
    ...
    try {
      switch (split.length) {
        case 1:
          ip = Parser.numStrToBigInt(split[0])
          break
        case 2:
          ip = Parser._handleIpSplit2(split)
          break
        case 3:
          ip = Parser._handleIpSplit3(split)
          break
        case 4:
          ip = Parser._handleIpSplit4(split)
          break
      }
    } catch (e) {
      return null
   ...
   * @private
   */
  static _handleIpSplit2 (splitArray) {
    let last = Parser.numStrToBigInt(splitArray[1])
    if (last.greaterOrEquals(twoTo24)) return null
    return Parser.numStrToBigInt(splitArray[0])
      .times(twoTo24)
      .add(last)
  }
   ...
   * @private
   */
  static _handleIpSplit3 (splitArray) {
    let middle = Parser.numStrToBigInt(splitArray[1])
    if (middle.greaterOrEquals(twoTo8)) return null
    let last = Parser.numStrToBigInt(splitArray[2])
   ...
   * @private
   */
  static _handleIpSplit4 (splitArray) {
    const p1 = Parser.numStrToBigInt(splitArray[1])
    const p2 = Parser.numStrToBigInt(splitArray[2])
    const p3 = Parser.numStrToBigInt(splitArray[3])
    if (
...
module.exports = { ParsedUrl, Parser }
